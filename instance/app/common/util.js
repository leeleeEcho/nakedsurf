...
function checkJWT(ctx) {
    ...
    let jwtSecret = require('nakedsurf').Instance.app.conf.app.jwtSecret || undefined;
    let decodeToken = require('nakedsurf').Utility.JWT.verify(token, jwtSecret);
...
function getNodeResourceUri(nodeId, hashName) {
    return "http://" + path.join(nakedsurfConfig[env].domain, "/attachment/", nodeId || "", hashName || "");
}
